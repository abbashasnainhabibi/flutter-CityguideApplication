...
import 'package:cityguide/ADMIN_PANEL/Mountain/MountainDatabase.dart';
import 'package:cityguide/ADMIN_PANEL/Appbar.dart';

class AddMountain extends StatefulWidget {
  const AddMountain({super.key});

  @override
  _AddMountainState createState() => _AddMountainState();
}

class _AddMountainState extends State<AddMountain> {
  final TextEditingController mountainNameController = TextEditingController();
  ...
  final MountainDatabase mountainDatabase = MountainDatabase();
  ...
  Future<void> _fetchProvinces() async {
    try {
      ...
      setState(() {
        provinces = snapshot.docs.map((doc) => doc['cityName'] as String).toList();
  ...
  Future<void> _addMountainToFirestore() async {
    String mountainName = mountainNameController.text.trim();
    ...
    try {
      await mountainDatabase.addMountainWithImages(
        MountainName: mountainName,
        description: description,
        latitude: lat,
        longitude: long,
        Mountainlocation: selectedProvince!,
        base64Images: base64Images,
      );

      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Mountain added successfully!'),
        behavior: SnackBarBehavior.floating,
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Error adding mountain. Please try again later.'),
        behavior: SnackBarBehavior.floating,
        backgroundColor: Colors.redAccent,
      ));
    }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                _buildTextField('Mountain Name', mountainNameController),
                SizedBox(height: 16),
                ...
                SizedBox(height: 24.0),
                 Center(
                  child: ElevatedButton(
                    onPressed: _addMountainToFirestore,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.green,
