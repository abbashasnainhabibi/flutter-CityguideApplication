...
import 'package:cityguide/Hotel/ParticularHotel.dart';
...
class AllHotel extends StatefulWidget {
  ...
  @override
  _AllHotelState createState() => _AllHotelState();
}
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('No Hotel available.'));
        }
        ...
        if (_searchQuery.isNotEmpty) {
          cities = cities.where((city) {
            var cityName = city['HotelName']?.toString().toLowerCase() ?? '';
            return cityName.contains(_searchQuery);
          }).toList();
        }

        return cities.isEmpty
            ? const Center(child: Text('No matching Hotel found.'))
            : GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String name = city['HotelName'] ?? 'Unknown Beach';
