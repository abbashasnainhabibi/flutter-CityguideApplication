...
import 'dart:convert';
import 'package:cityguide/Mountains/ParticularMountain.dart';
...
class AllMountains extends StatefulWidget {
  const AllMountains({super.key});

  @override
  _AllMountainsState createState() => _AllMountainsState();
}

class _AllMountainsState extends State<AllMountains> {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('No Mountains available.'));
        }
        ...
        if (_searchQuery.isNotEmpty) {
          cities = cities.where((city) {
            var cityName = city['MountainName']?.toString().toLowerCase() ?? '';
            return cityName.contains(_searchQuery);
          }).toList();
        }

        return cities.isEmpty
            ? const Center(child: Text('No matching Mountains found.'))
            : GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String name = city['MountainName'] ?? 'Unknown Mountain';
